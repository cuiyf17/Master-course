...
class cuiyfStatsReal(StatsBase):
    def __init__(self, cfg, alphaNode):
        ...
        if(self.dump_group_pnl):
            ...
            if not os.path.isdir(group_retDir):
                os.makedirs(group_retDir)
            pnlName = self.alphaNode.alphaId
            group_retFile = group_retDir + '/' + pnlName
            self.groupret_output = open(group_retFile, 'r+' if os.path.isfile(group_retFile) and cpt.cptdir!='' else 'w', buffering=4096)

            group_pnlDir = cfg.getAttributeStringDefault('group_pnlDir', './group_pnl')
            if not os.path.isdir(group_pnlDir):
                os.makedirs(group_pnlDir)
            pnlName = self.alphaNode.alphaId
            group_pnlFile = group_pnlDir + '/' + pnlName
            self.grouppnl_output = open(group_pnlFile, 'r+' if os.path.isfile(group_pnlFile) and cpt.cptdir!='' else 'w', buffering=4096)

            longshort_pnlDir = cfg.getAttributeStringDefault('longshort_pnlDir', './longshort_pnl')
            if not os.path.isdir(longshort_pnlDir):
                os.makedirs(longshort_pnlDir)
            pnlName = self.alphaNode.alphaId
            longshort_pnlFile = longshort_pnlDir + '/' + pnlName
            self.longshortpnl_output = open(longshort_pnlFile, 'r+' if os.path.isfile(longshort_pnlFile) and cpt.cptdir!='' else 'w', buffering=4096)
    ...
    def calculate(self, di):
        ...
        # pnl
        if(self.dump_group_pnl):
            alpha_value = self.oPos[~np.isnan(self.oPos)]
            alpha_ret = _ret[~np.isnan(self.oPos)]
            old_group = OP.group_split2(alpha_value, self.num_groups)
            # new_group = OP.group_split2(today_dollar_weight, self.num_groups)
            group_ret = np.zeros(self.num_groups+1)
            for i in range(self.num_groups):
                idx = (old_group == i)
                group_ret[i] = np.nanmean(alpha_ret[idx])#np.nansum(yesterday_dollar_weight[idx]*_ret[idx])
                #idx = new_group == i
                #group_pnl[i] += np.nanmean( (self.cps[di, idx] / trade_price[idx] - 1))#np.nansum(trades[idx] * (self.cps[di, idx] / trade_price[idx] - 1))
                #group_pnl[i] -= np.nansum(np.fabs(trades[idx] * np.where(trades[idx] > 0, self.fee / 10000., (self.fee + self.tax) / 10000.)))
            group_ret[self.num_groups] = self.zz500[di] / self.zz500[di-1] - 1

            old_group = OP.group_split2(yesterday_dollar_weight, self.num_groups)
            new_group = OP.group_split2(today_dollar_weight, self.num_groups)
            group_pnl = np.zeros(self.num_groups)
            for i in range(self.num_groups):
                idx = (old_group == i)
                group_pnl[i] += np.nansum(yesterday_dollar_weight[idx]*_ret[idx])
                idx = new_group == i
                group_pnl[i] += np.nansum(trades[idx] * (self.cps[di, idx] / trade_price[idx] - 1))
                group_pnl[i] -= np.nansum(np.fabs(trades[idx] * np.where(trades[idx] > 0, self.fee / 10000., (self.fee + self.tax) / 10000.)))
            
            booksize = np.nansum(np.fabs(yesterday_dollar_weight))
            zz500_ret = self.zz500[di] / self.zz500[di-1] - 1
            zz500_pnl = zz500_ret * booksize
            longshort_pnl = np.zeros(2)
            for i in range(2):
                idx = yesterday_dollar_weight >= 0 if i == 0 else yesterday_dollar_weight < 0
                longshort_pnl[i] += np.nansum((-1)**i*yesterday_dollar_weight[idx]*_ret[idx]) - zz500_pnl
        ...
        if self.dump_group_pnl:
            group_retLine = '%d %s\n'%(uv.Dates[di], ' '.join([str(x) for x in group_ret]))
            self.groupret_output.write(group_retLine)
            
            group_pnlLine = '%d %s\n'%(uv.Dates[di], ' '.join([str(x) for x in group_pnl]))
            self.grouppnl_output.write(group_pnlLine)

            longshort_pnlLine = '%d %s\n'%(uv.Dates[di], ' '.join([str(x) for x in longshort_pnl]))
            self.longshortpnl_output.write(longshort_pnlLine)
        return
        ...
        if(self.dumpPnl):
            ...
            while line:
                ...
                if dt == uv.Dates[cpt.savedi]:
                    ...
                    line = self.output.readline()
        if(self.dump_group_pnl):
            line = self.groupret_output.readline()
            while line:
                dt = int(line.split()[0])
                if dt == uv.Dates[cpt.savedi]:
                    self.groupret_output.seek(self.groupret_output.tell())
                    break
                else:
                    line = self.groupret_output.readline()

            line = self.grouppnl_output.readline()
            while line:
                dt = int(line.split()[0])
                if dt == uv.Dates[cpt.savedi]:
                    self.grouppnl_output.seek(self.grouppnl_output.tell())
                    break
                else:
                    line = self.grouppnl_output.readline()

            line = self.longshortpnl_output.readline()
            while line:
                dt = int(line.split()[0])
                if dt == uv.Dates[cpt.savedi]:
                    self.longshortpnl_output.seek(self.longshortpnl_output.tell())
                    break
                else:
                    line = self.longshortpnl_output.readline()
        return
