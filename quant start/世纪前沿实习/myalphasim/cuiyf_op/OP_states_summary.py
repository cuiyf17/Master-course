...
class cuiyfStatesSummary(AlphaOpBase):
    def __init__(self, cfg):
        ...
        with open(self.alpha_path, 'w') as f:
            f.write('')
        self.booksize = float(cfg.getConfig().getparent().getparent().getparent().attrib['booksize'])
    ...
    def apply(self, di, alpha):
        ...
        # self.booksize += self.pnl
        alphasum = np.nansum(np.abs(tmp_alpha))
        if alphasum > 1:
            tmp_alpha = tmp_alpha / alphasum
        tmp_alpha = tmp_alpha * self.booksize

        preclose = self.cps[di - 1] * self.adj[di]
        nPos = tmp_alpha  # stats is not supposed to change alpha vector
        # limit hitting & halt
        ...
        self.hld_val = self.booksize #np.nansum(val_h)
        ...
        # turnover
        self.cumHldVal += self.booksize#self.yestlong - self.yestshrt
        ...
        if self.num > 0 and self.yestlong - self.yestshrt > 0:
            self.ret = 2 * self.pnl / self.booksize#2 * self.pnl / (self.yestlong - self.yestshrt)
        ...
        # screen output
        if self.printStats:
            print("%8s%32s%12s   %-10s %-5s x %5s %10s %10s %12s %7s %7s %7s %10s" % (" ", " ", " ", " ", "LONG", "SHORT", "SHARES", "PNL", "CUMPNL", "TVR", "RET", "DD", "IR"))
            print('%8d%32s%12d x %-10d %-5s x %5s %10d %10d %12d %7.3f %7.3f %7.3f %10.3f' % (
            uv.Dates[di], self.alphaname, int(self.long_value),
