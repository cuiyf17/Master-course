...
class Alpha(AlphaBase):
    def __init__(self, cfg):
        ...
        # # self.research_inst_id = dr.getData("WIND_AShareEarningEst.RESEARCH_INST_ID")
    ...
    def generate(self, di):
        ...
        industry = self.subindustry[di-self.delay]
        ...
        g_returns = self.adj_close[di-self.delay]/self.adj_close[di-self.delay-21]-1
        g_std = OP.std(self.adj_close[start_di:end_di]/self.adj_close[start_di-1:end_di-1]-1)
        g_turnover = self.amount[di-self.delay]/self.negcap[di-self.delay]
        g_vol1 = (self.adj_open[di-self.delay]/self.adj_close[di-self.delay-1]-1)*(self.adj_close[di-self.delay]/self.adj_open[di-self.delay]-1)
        g_vol2 = self.high[di-self.delay]/self.low[di-self.delay]-1
        g_skew = OP.skew(self.adj_close[start_di:end_di]/self.adj_close[start_di-1:end_di-1]-1)
        g_kurt = OP.kurt(self.adj_close[start_di:end_di]/self.adj_close[start_di-1:end_di-1]-1)
        nan_mask = ~(np.isnan(asset_tvr) | np.isnan(gross_profit) | np.isnan(receivable) | np.isnan(pe_ratio) | np.isnan(g_returns) | np.isnan(g_std) | np.isnan(g_turnover) | np.isnan(g_vol1) | np.isnan(g_vol2) | np.isnan(g_skew) | np.isnan(g_kurt))
        nan_where = np.where(nan_mask)[0]
        features = np.concatenate([industry.reshape((-1,1)), pe_ratio.reshape(-1,1), asset_tvr.reshape((-1,1)), gross_profit.reshape((-1,1)), receivable.reshape((-1,1)), g_returns.reshape((-1,1)), g_std.reshape((-1,1)), g_turnover.reshape((-1,1)), g_vol1.reshape((-1,1)), g_vol2.reshape((-1,1)), g_skew.reshape((-1,1)), g_kurt.reshape((-1,1))], axis=1)
        pca = PCA(n_components=1, random_state=42)
        pca.fit(features[nan_where, :])
        g_cluster = pca.transform(features[nan_where, :]).reshape(-1)
        group = OP.group_split2(g_cluster, 40)
        real_group = np.zeros_like(self.close[di-self.delay])
        real_group[nan_where] = group
        real_group[~nan_mask] = np.nan

        grp = real_group # self.subindustry[di-self.delay]

        amount = self.amount[di-self.delay]
        act_buy_amount = self.buy_large_act[di-self.delay] + self.buy_medium_act[di-self.delay] + self.buy_exlarge_act[di-self.delay]# + self.buy_small_act[di-self.delay]
        act_sell_amount = self.sell_large_act[di-self.delay] + self.sell_medium_act[di-self.delay] + self.sell_exlarge_act[di-self.delay]# + self.sell_small_act[di-self.delay]
        net_act_buy_amount = (act_buy_amount - act_sell_amount)/amount


        ret = self.adj_close[di-self.delay]/self.adj_close[di-self.delay-1] - 1
        tmp = np.zeros((1, self.close[di-self.delay].shape[0]))
        tmp[0] = OP.group_mean(net_act_buy_amount, grp)
        if(self.group_strength is None):
            self.group_strength = tmp
        elif(self.group_strength.shape[0] < 21):
            self.group_strength = np.concatenate((self.group_strength, tmp), axis=0)
        else:
            self.group_strength = np.concatenate((self.group_strength[1:], tmp), axis=0)

        ret21 = self.adj_close[di-self.delay]/self.adj_close[di-self.delay-21] - 1
        tmp = np.zeros((1, self.close[di-self.delay].shape[0]))
        tmp[0] = OP.group_mean(ret21, grp)
        if(self.group_ret is None):
            self.group_ret = tmp
        elif(self.group_ret.shape[0] < self.group_strength.shape[0]):
            self.group_ret = np.concatenate((self.group_ret, tmp), axis=0)
        else:
            self.group_ret = np.concatenate((self.group_ret[1:], tmp), axis=0)
        sorted_idxs = np.argsort(self.group_ret, axis=0)
        argmax1_ret21 = sorted_idxs[-1, :]
        group_strength1 = self.group_strength[argmax1_ret21, np.arange(num_stocks)]
        argmax2_ret21 = 0
        argmax3_ret21 = 0
        if(self.group_strength.shape[0] > 1):
            argmax2_ret21 = sorted_idxs[-2, :]
            group_strength2 = self.group_strength[argmax2_ret21, np.arange(num_stocks)]
        
        # argmax_ret21 = np.argmax(ret21, axis=0)
        if(self.group_strength.shape[0] > 1):
            group_strength = OP.mean(np.array([group_strength1, group_strength2]), axis=0)
        else:
            group_strength = group_strength1
        alpha = group_strength
        

        # zscore开仓平仓
        # alpha = self.zscore_position(alpha, di, openthres = 0.2, holdthres = 0.1, use_tail = True)
        # 止盈止损
        # alpha = self.corr_stop_profit_loss(alpha, di, lookback = 5, stop_profit = 1, stop_loss = -0.2, excess_zz500 = True, continuous = False)
        # 决定总仓位大小
    ...
    def checkpointSave(self, fh):
        ...
        pickle.dump(self.group_strength, fh)
        pickle.dump(self.group_ret, fh)
    ...
    def checkpointLoad(self, fh):
        ...
        self.group_strength = pickle.load(fh)
        self.group_ret = pickle.load(fh)
