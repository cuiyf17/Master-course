...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        iopen = self.iopen[start_di:end_di, :7,  valid_idx]
        iclose = self.iclose[start_di:end_di, 6,  valid_idx]
        ihigh = self.ihigh[start_di:end_di, :7,  valid_idx]
        ilow = self.ilow[start_di:end_di, :7,  valid_idx]
        open_ret = iclose/self.open[start_di:end_di, valid_idx]-1
        open_ret = open_ret - np.nanmean(open_ret, axis=1, keepdims=True)
        open_vol = np.nanmax(ihigh, axis=1)/np.nanmin(ilow, axis=1)
        open_vol = open_vol / np.nanmean(open_vol, axis=1, keepdims=True) - 1
        # print(alpha.shape)
        alpha = OP.zscore(open_ret*open_vol)[-1]
        # alpha = OP.rank(alpha) - 0.5
        # alpha = OP.power(alpha, 3)
        # alpha = np.where(np.abs(alpha) > 0.45, np.sign(alpha), np.nan)/alpha.shape[0]*10

        # 根据zscore开仓平仓
        alpha = self.zscore_position(alpha, di, valid_idx, openthres = 0.5, holdthres = 0., use_tail = True)
        alpha = alpha
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, valid_idx, lookback = 5, stop_profit = 1, stop_loss = -0.7, excess_zz500 = True, continuous = True)
        # 动量更新
