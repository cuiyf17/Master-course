...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        if(is_reverse):
            ...
            # rets = (rets - np.nanmean(rets, axis=1, keepdims=True))/np.nanstd(rets, axis=1, keepdims=True)
            ...
            if(self.pre_alpha is None):
                ...
                self.holding_days += 0
                longer = np.where(self.holding_days > 63)
                alpha[longer] = 0
        ...
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, lookback = 30, stop_profit = 0.5, stop_loss = -1, excess_zz500 = True, continuous = False)
        # 决定总仓位大小
