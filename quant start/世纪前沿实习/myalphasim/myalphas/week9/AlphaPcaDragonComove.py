...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        nan_mask = ~(np.isnan(g_returns) | np.isnan(g_std) | np.isnan(g_turnover) | np.isnan(g_vol1) | np.isnan(g_vol2) | np.isnan(g_skew) | np.isnan(g_kurt))
        nan_where = np.where(nan_mask)[0]

        features = np.concatenate([industry.reshape((-1,1)), g_returns.reshape((-1,1)), g_std.reshape((-1,1)), g_turnover.reshape((-1,1)), g_vol1.reshape((-1,1)), g_vol2.reshape((-1,1)), g_skew.reshape((-1,1)), g_kurt.reshape((-1,1))], axis=1)
        ...
        real_group = np.zeros_like(g_returns)
        ...
        # cap = self.cap[di-self.delay]
        # cap_zscore = OP.groupNeutralize(cap, grp)
        ...
        # alpha = alpha*2
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, lookback = 5, stop_profit = 1, stop_loss = -0.8, excess_zz500 = True, continuous = False)
        # 动量更新
        alpha = alpha[valid_idx]
        self.momentum_update(alpha, di, valid_idx, trade_interval = 1, momentum = 0.2)
