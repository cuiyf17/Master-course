...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        iclose = self.iclose[start_di:end_di, -12]
        close_ret = self.close[start_di:end_di]/iclose - 1
        close_ret = close_ret - np.nanmean(close_ret, axis=1, keepdims=True)
        # print(close_ret.shape)
        # print(alpha.shape)
        alpha = -close_ret
        alpha = (alpha[-1] -OP.mean(alpha))/OP.std(alpha)
        ...
        alpha = alpha*2
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, lookback = 5, stop_profit = 0.8, stop_loss = -0.6, excess_zz500 = True, continuous = True)
        # 动量更新
