...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        ret21 = self.adj_close[di-self.delay-self.group_strength.shape[0]+1:di-self.delay+1]/self.adj_close[di-self.delay-self.group_strength.shape[0]:di-self.delay] - 1
        sorted_idxs = np.argsort(ret21, axis=0)
        ...
        # alpha = OP.mean(self.group_strength)
        # alpha = alpha #/ OP.std(self.group_strength)

        # zscore开仓平仓
        # alpha = self.zscore_position(alpha, di, openthres = 0.4, holdthres = 0.1, use_tail = True)
        # 止盈止损
        # alpha = self.corr_stop_profit_loss(alpha, di, lookback = 10, stop_profit = 0.8, stop_loss = -0.8, excess_zz500 = True, continuous = True)
        # 动量更新
