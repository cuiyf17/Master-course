...
class Alpha(AlphaBase):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.ndays = cfg.getAttributeDefault('ndays', 160)
    ...
    def generate(self, di):
        ...
        zz500_ret = (self.zz500[start_di:end_di]/self.zz500[start_di-1:end_di-1] - 1).reshape((-1, 1))
        excess_ret = self.close[start_di:end_di, valid_idx]/self.close[start_di-1:end_di-1, valid_idx]-1 - zz500_ret
        high = self.high[start_di:end_di, valid_idx]
        low = self.low[start_di:end_di, valid_idx]
        close = self.close[start_di-1:end_di-1, valid_idx]
        # open = self.open[start_di:end_di, valid_idx]
        volatility = (high - low)/close
        returns = self.close[start_di:end_di, valid_idx]/self.close[start_di-1:end_di-1, valid_idx]-1
        mean_vol = np.nanmean(volatility, axis=1, keepdims=True)
        std_vol = np.nanstd(volatility, axis=1, keepdims=True)
        threshold = np.minimum(mean_vol+std_vol, 0.05)
        mask = np.where((volatility <= threshold) & (np.abs(returns)<=0.095), 1, np.nan)
        alpha = np.nanmean(excess_ret*mask, axis=0)
        ...
        # 根据zscore开仓平仓
        alpha = self.zscore_position(alpha, di, valid_idx, openthres = 0.6, holdthres = 0.4, use_tail = False)
        alpha = alpha*8
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, valid_idx, lookback = 5, stop_profit = 0.7, stop_loss = -0.7, excess_zz500 = True, continuous = True)
        # 动量更新
        self.momentum_update(alpha, di, valid_idx, trade_interval = 1, momentum = 0.05)
    ...
    def zscore_position(self, alpha, di, valid_idx, openthres = 0.6, holdthres = 0.4, use_tail = False):
        ...
        zscore = (alpha - np.nanmean(self.pre_alphas_zscore[:, valid_idx], axis=0))/np.nanstd(self.pre_alphas_zscore[:, valid_idx], axis=0)
        zscore = np.clip(zscore, -3, 3)/3
        if(self.pre_alpha is not None):
            choose_where = ((np.abs(zscore) > openthres) & (np.isnan(self.pre_alpha[valid_idx]))) | ((np.abs(zscore) > holdthres) & (~np.isnan(self.pre_alpha[valid_idx])))
        else:
    ...
    def corr_stop_profit_loss(self, alpha, di, valid_idx, lookback = 5, stop_profit = 0.7, stop_loss = -0.7, excess_zz500 = True, continuous = True):
        tmp = np.zeros((1, self.close[di-1].shape[0]))
        tmp[0, valid_idx] = alpha
        ...
        if(excess_zz500):
            ...
            ex_ret = self.close[di-self.delay-nn+1:di-self.delay+1, valid_idx]/self.close[di-self.delay-nn:di-self.delay, valid_idx] - 1 - zz500_ret
        else:
            ex_ret = self.close[di-self.delay-nn+1:di-self.delay+1, valid_idx]/self.close[di-self.delay-nn:di-self.delay, valid_idx] - 1
        alpha_momentum = OP.corr(ex_ret, self.pre_alphas[:, valid_idx])
        if(continuous):
    ...
    def momentum_update(self, alpha, di, valid_idx, trade_interval = 1, momentum = 0.05):
        if(self.pre_alpha is None):
            ...
            if((di - self.delay)%trade_interval == 0):
                self.alpha[valid_idx&mask2] = 0.95*self.pre_alpha[valid_idx&mask2] + 0.05*alpha[mask1]
