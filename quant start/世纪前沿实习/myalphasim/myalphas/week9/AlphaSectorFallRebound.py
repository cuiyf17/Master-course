...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        elif(self.sector_indexs.shape[0] < 25):
        ...
        vol = OP.mean(self.sector_atr)
        adj_coef = np.ones_like(self.close[di-self.delay])
        adj_coef = np.where(vol > 0.02, np.sqrt(vol/0.02), adj_coef)
        adj_coef = np.where(vol < 0.01, np.sqrt(vol/0.01), adj_coef)
        thres_fall = -0.10*adj_coef
        thres_rise = 0.01*adj_coef

        sectors = np.unique(self.sector[di-self.delay])
        for sec in sectors:
            sec_idx = np.where(self.sector[di-self.delay] == sec)[0]
            sec_tres_fall = np.nanmean(thres_fall[sec_idx])
            sec_tres_rise = np.nanmean(thres_rise[sec_idx])
            is_reverse = False
            if(self.sector_indexs.shape[0] >= 5):
                sec_index_ret = np.nanmean(self.sector_indexs[-1, sec_idx]/self.sector_indexs[-2, sec_idx]-1)
                if(sec_index_ret > sec_tres_rise):
                    lookback = 0
                    rise = 0
                    while(rise <= sec_tres_rise and lookback < min(21, self.sector_indexs.shape[0]-2)):
                        lookback += 1
                        rise = np.nanmean(self.sector_indexs[-lookback-1, sec_idx]/self.sector_indexs[-lookback-2, sec_idx])-1
                    if(lookback > 4):
                        sector_index_prices = np.nanmean(self.sector_indexs[-lookback-1:-1, sec_idx], axis=1)
                        # print(self.sector_indexs[-lookback-1:-1, sec_idx].shape)
                        # print(sector_index_prices.shape)
                        sector_index_argmax = np.argmax(sector_index_prices)
                        sector_index_max = sector_index_prices[sector_index_argmax]
                        sector_index_argmin = np.argmin(sector_index_prices)
                        sector_index_min = sector_index_prices[sector_index_argmin]
                        if((sector_index_argmax - sector_index_argmin < -2)):
                            fall_step = 1 - sector_index_min/sector_index_max
                            if(fall_step > sec_tres_fall):
                                is_reverse = True
            ...
            if(is_reverse):
                ...
                alpha[sec_idx] = ((rets[-1,:] - OP.mean(rets))/OP.std(rets)*OP.moment(rets, 4))[sec_idx]
                group = OP.group_split2(alpha[sec_idx], 10)
                alpha[sec_idx] = OP.group_mean(alpha[sec_idx], group)
                if(self.holding_days is None):
                    self.holding_days = np.zeros_like(self.close[di-self.delay])
                else:
                    self.holding_days[sec_idx] = 0
            else:
                if(self.pre_alpha is None):
                    ...
                    alpha[sec_idx] = self.pre_alpha[sec_idx]
                    if(self.holding_days is None):
                        self.holding_days = np.zeros_like(self.close[di-self.delay])
                    else:
                        self.holding_days[sec_idx] += 1
