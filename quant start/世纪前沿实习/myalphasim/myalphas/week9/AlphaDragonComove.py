...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        # g_volume = OP.sum(self.volume[start_di:end_di])
        ...
        # 根据zscore开仓平仓
        # alpha = self.zscore_position(alpha, di, openthres = 0.5, holdthres = 0., use_tail = True)
        # alpha = alpha*2
        # 止盈止损
        alpha = self.corr_stop_profit_loss(alpha, di, lookback = 5, stop_profit = 1, stop_loss = -0.5, excess_zz500 = True, continuous = True)
        # 动量更新
        alpha = alpha[valid_idx]
        self.momentum_update(alpha, di, valid_idx, trade_interval = 1, momentum = 0.5)
