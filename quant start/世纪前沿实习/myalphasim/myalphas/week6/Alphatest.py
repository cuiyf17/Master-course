...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        alpha = self.vwap[di-self.delay, valid_idx]/self.close[di-self.delay, valid_idx] - 1
        alpha = OP.rank(alpha) - 0.5
        alpha = np.where(np.abs(alpha)>0.45, alpha, np.nan)/alpha.shape[0]
