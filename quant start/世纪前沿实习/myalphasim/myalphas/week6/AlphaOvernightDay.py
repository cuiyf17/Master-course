...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        overnight = self.open[start_di:end_di, valid_idx] - self.close[start_di-1:end_di-1, valid_idx]
        for i in range(overnight.shape[0]):
            overnight[i] = overnight[i] - OP.group_mean(overnight[i], group2)
        dayret = self.close[start_di:end_di, valid_idx] - self.open[start_di:end_di, valid_idx]
        for i in range(dayret.shape[0]):
            dayret[i] = dayret[i] - OP.group_mean(dayret[i], group2)

        alpha = -OP.corr(overnight, dayret)#*OP.std(dayret)*OP.std(overnight)
        alpha = OP.groupNeutralize(alpha, group3)
        alpha = OP.groupNeutralize(alpha, group2, standardization=False)
        # alpha = 3*OP.rank(alpha) - 1.5
