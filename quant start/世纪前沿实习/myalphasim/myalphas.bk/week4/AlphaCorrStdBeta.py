...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        g_returns = self.close[di-self.delay, valid_idx]/self.close[di-self.delay-126, valid_idx] - 1
        g_close = self.close[di-self.delay, valid_idx]
        group1 = OP.group_split2(g_returns, 5)
        group2 = OP.group_split2(g_close, 10)

        returns1 = self.close[di-self.delay-21+1:di-self.delay+1, valid_idx] / self.close[di-self.delay-21:di-self.delay, valid_idx] - 1
        #returns1 = returns1 - np.nanmean(returns1, axis=1, keepdims=True)
        beta_rets = np.nanmean(returns1, axis=1).repeat(returns1.shape[1]).reshape(-1, returns1.shape[1])
        std_returns1 = OP.std(returns1)
        std_returns2 = OP.std(beta_rets)

        tmp = np.zeros((1, self.close[di].shape[0]))
        tmp[0, valid_idx] = std_returns1
        tmp2 = np.zeros((1, self.close[di].shape[0]))
        tmp2[0, valid_idx] = std_returns2
        if(self.std_returns1 is None):
            self.std_returns1 = tmp.copy()
            self.std_returns2 = tmp2.copy()
        elif(self.std_returns1.shape[0] < 21):
            self.std_returns1 = np.concatenate([self.std_returns1, tmp], axis=0)
            self.std_returns2 = np.concatenate([self.std_returns2, tmp2], axis=0)
        else:
            self.std_returns1 = np.concatenate([self.std_returns1[1:], tmp], axis=0)
            self.std_returns2 = np.concatenate([self.std_returns2[1:], tmp2], axis=0)
        
        returns = self.close[di-self.delay, valid_idx]/self.close[di-self.delay-21, valid_idx] - 1
        #beta_std = np.nanmean(self.std_returns1[:, valid_idx], axis=1).repeat(self.std_returns1[:, valid_idx].shape[1]).reshape(-1, self.std_returns1[:, valid_idx].shape[1])
        corr = OP.neutralizeVector(std_returns1, returns)
        #corr = OP.ts_neutralize_vector(self.std_returns2[:, valid_idx], self.std_returns1[:, valid_idx])
        alpha = -corr*OP.moment(returns1, 4)

        alpha = OP.groupNeutralize(alpha, group1)
        # alpha = OP.power(alpha, 1)
