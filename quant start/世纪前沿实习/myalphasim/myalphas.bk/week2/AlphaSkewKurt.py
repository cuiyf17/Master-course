...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        skew_close = cuiyfOp.skew(self.close[di - self.delay - 21 + 1:di - self.delay + 1, valid_idx])
        std_close = cuiyfOp.std(self.close[di - self.delay - 21 + 1:di - self.delay + 1, valid_idx])
        mean_close = cuiyfOp.mean(self.close[di - self.delay - 21 + 1:di - self.delay + 1, valid_idx])
        kurt_close = cuiyfOp.kurt(self.close[di - self.delay - 63 + 1:di - self.delay + 1, valid_idx])
        #std_close = cuiyfOp.std(self.close[start_di + self.ndays: end_di + self.lag, valid_idx])
        
        alpha = -(cuiyfOp.rank(skew_close)*cuiyfOp.rank(kurt_close))
        ...
        # alpha = cuiyfOp.groupNeutralize(alpha, group2, standardization=False)
        alpha = cuiyfOp.groupNeutralize(alpha, group3, standardization=True)
