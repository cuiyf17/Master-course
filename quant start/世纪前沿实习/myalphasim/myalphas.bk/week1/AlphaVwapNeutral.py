...
class AlphaVwapClose(AlphaBase):
    ...
    def generate(self, di):
        ...
        returns = self.close[di - self.delay, valid_idx] / self.close[di - self.delay - 252, valid_idx]
        my_groups = cuiyfOp.group_split(returns, 5)
        
        alpha = self.vwap[start_di : end_di, valid_idx] / self.close[start_di : end_di, valid_idx]
        alpha = cuiyfOp.zscore(alpha)[-1, :]
        alpha = cuiyfOp.groupNeutralize(alpha, my_groups)
        
        #reversal = self.vwap[di - self.delay, valid_idx] / self.close[di - self.delay, valid_idx]
        reversal = 1 - self.close[di - self.delay, valid_idx] / self.close[di - self.delay - 5, valid_idx]
        Oputil.rank(reversal)
        Oputil.power(reversal, 3)
        Oputil.neutralize(alpha, reversal)
