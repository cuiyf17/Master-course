...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        g_returns = self.close[di-self.delay, valid_idx]/self.close[di-self.delay-252, valid_idx] - 1
        ...
        returns = self.close[start_di:end_di, valid_idx]/self.close[start_di-252:end_di-252, valid_idx] - 1
        #returns = returns - OP.group_mean(returns, industry)
        volume1 = self.volume[start_di:end_di, valid_idx]
        volume2 = self.volume[start_di+self.ndays:end_di+self.lag, valid_idx]
        coef = OP.mean(volume2)/OP.mean(volume1)

        alpha = returns[-1] - OP.group_mean(returns[-1], group3)
        alpha = alpha * coef #* OP.moment(returns,4)
        alpha = OP.rank(alpha)
        alpha = OP.power(alpha, 0.25)
