...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        overnight = self.open[di-self.delay, valid_idx]/self.close[di-self.delay-1, valid_idx] - 1
        alpha = overnight
        alpha = OP.rank(alpha)
        ...
        if(self.pre_alpha is None):
            ...
            if((di - self.delay)%1 == 0):
                self.alpha[valid_idx&mask2] = 0.89*self.pre_alpha[valid_idx&mask2] + 0.11*alpha[mask1]
