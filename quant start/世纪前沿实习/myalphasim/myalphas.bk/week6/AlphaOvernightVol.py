...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        overnight = self.open[start_di:end_di, valid_idx] / self.close[start_di-1:end_di-1, valid_idx]-1
        for i in range(overnight.shape[0]):
            overnight[i] = overnight[i] - OP.group_mean(overnight[i], group2)
        dayret = self.close[start_di:end_di, valid_idx] / self.open[start_di:end_di, valid_idx]-11
        for i in range(dayret.shape[0]):
            dayret[i] = dayret[i] - OP.group_mean(dayret[i], group2)

        alpha = -OP.mean(overnight*dayret)*OP.corr(overnight, dayret)
