...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        openclose = np.concatenate([open.reshape((self.ndays,-1,1)), close.reshape((self.ndays,-1,1))], axis=2)
        upshadow = (high - np.nanmax(openclose, axis=2))/close
        downshadow = (np.nanmin(openclose, axis=2) - low)/close
        alpha = -OP.corr(upshadow, downshadow)# * OP.moment(downshadow, 4)
