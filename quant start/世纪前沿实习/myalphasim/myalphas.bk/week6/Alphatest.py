...
class Alpha(AlphaBase):
    ...
    def generate(self, di):
        ...
        group4 = OP.group_split2(g_amount, 10)
        group5 = OP.group_split2(g_std, 10)


        amount = self.amount[start_di : end_di, valid_idx]
        std_amount = OP.std(amount)
        tmp = np.zeros((1, len(self.close[di-self.delay]))) + np.nan
        tmp[0, valid_idx] = std_amount
        if(self.std_amount is None):
            self.std_amount = tmp.copy()
        elif(self.std_amount.shape[0] < 63):
            self.std_amount = np.concatenate((self.std_amount, tmp), axis=0)
        else:
            self.std_amount = np.concatenate((self.std_amount[1:], tmp), axis=0)

        std_amount = OP.zscore(self.std_amount)[-1]
        alpha = std_amount
        ...
        if(self.pre_alpha is None):
            ...
            if((di - self.delay)%1 == 0):
                self.alpha[valid_idx&mask2] = 0.7*self.pre_alpha[valid_idx&mask2] + 0.3*alpha[mask1]
    ...
    def checkpointSave(self, fh):
        ...
        pickle.dump(self.std_amount, fh)



    def checkpointLoad(self, fh):
        self.pre_alpha = pickle.load(fh)
        self.std_amount = pickle.load(fh)
